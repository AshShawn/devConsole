...
(function ($) {
    function init(param, obj) {
        ...
        function createPageBar() {
            ...
            var b = opts.currPage, c = parseInt(opts.pageCount), d = parseInt(opts.pageNumber / 2), e = opts.pageNumber,
                f = "";
            ...
            opts.panel.next_on && (f += b == c ? '<span class="disabled" paged="' + opts.panel.next + '">' + opts.panel.next + " </span>" : "<a href='" + a + "' paged='" + (b + 1) + "'>" + opts.panel.next + " </a>"), opts.panel.last_on && (f += "<a href='" + a + "' paged='" + c + "'>" + opts.panel.last + "</a>"), f += getPanelTipInfo(), debug(opts.id), debug("\u6700\u7ec8\u751f\u6210\u83dc\u5355\uff1a"), debug(f), obj.html(f), obj.children("select").val(opts.currPage), obj.children("select").change(function () {
